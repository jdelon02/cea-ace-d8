...
if (defined('PANTHEON_ENVIRONMENT')) {
  ...
  $siteName = '-cea-ace-d8.pantheonsite.io';

  // This assumes https.
  $civiDomain = 'https://' . PANTHEON_ENVIRONMENT . $siteName;

  // Extract Pressflow settings into a php object.
  $pressflow_settings = json_decode($_SERVER['PRESSFLOW_SETTINGS']);
  
  // Drupal Root Info
  $pantheonConf = $pressflow_settings->conf;
  $rootDir = "/srv/bindings/" . $pantheonConf->pantheon_binding;
  $webRoot = $rootDir . '/code/web';
  $fileRoot = $rootDir . '/files';
  $tempDir = $pantheonConf->file_temporary_path;
  $civicrmRootDir = $rootDir . "/code/vendor/civicrm/civicrm-core";
  
  // DB Info
  $pantheonDb = $pressflow_settings->databases->default->default;
  $dbString = $pantheonDb->driver . "://" . $pantheonDb->username . ":";
  $dbString .= $pantheonDb->password . "@" . $pantheonDb->host . ":";
  $dbString .= $pantheonDb->port . "/" . $pantheonDb->database . "?new_link=true";
  $crmString = $dbString;
  $cmsString = $dbString;

  // Redis info.
  $redHost = $pantheonConf->redis_client_host;
  $redPass = $pantheonConf->redis_client_password;
  $redPort = $pantheonConf->redis_client_port;
...
if (file_exists( __DIR__ . DIRECTORY_SEPARATOR . 'settings.eapps.php')) {
  ...
  /**
   * eApps Specific:
   * Populate needed variables based on the eApps environment.
   */
  
  /**
   * Staging detection and changes.
   */
  if (strpos(getcwd(), 'staging') !== false) {
    $civiDomain = 'https://www.staging.ceacisp.org';
    $crmString = 'mysql://cea_dba:SURkD5FGUxHaBYRjQDuS@127.0.0.1:3310/staging_civicrm?new_link=true';
    $cmsString = 'mysql://cea_dba:SURkD5FGUxHaBYRjQDuS@127.0.0.1:3310/staging_drupal?new_link=true';
    $rootDir = '/var/www/wwwadmin/data/www/staging.ceacisp.org';
  } else {
    $civiDomain = 'https://www.ceacisp.org';
    $crmString = 'mysql://cea_dba:SURkD5FGUxHaBYRjQDuS@127.0.0.1:3310/ceacisp_civicrm?new_link=true';
    $cmsString = 'mysql://cea_dba:SURkD5FGUxHaBYRjQDuS@127.0.0.1:3310/ceacisp_drupal?new_link=true';
    $rootDir = '/var/www/wwwadmin/data/www/ceacisp.org';
  }
   
  $civicrmRootDir = $rootDir . "/vendor/civicrm/civicrm-core";
  $webRoot = $rootDir . '/web';
  //$webRoot = $civiDomain . '/libraries/civicrm'; 
  $fileRoot = $webRoot . '/sites/default/files';
  $tempDir = $fileRoot . '/upload';
  ...
  /**
  * local:
  * Populate needed variables based on the local environment.
  */
}
...
if (!defined('CIVICRM_UF_DSN') && CIVICRM_UF !== 'UnitTests') {
  define( 'CIVICRM_UF_DSN'           , $cmsString);
}
...
$civicrm_paths['civicrm.root'] = array(
  'url' => $civiDomain . '/libraries/civicrm/',
  'path' => $civicrmRootDir,
);

// $civicrm_paths['civicrm.files'] = array(
//   // 'url' => $civiDomain,
//   'path' => '/sites/default/files/civicrm',
// );

$civicrm_root = $civicrmRootDir;
...
if (!defined('CIVICRM_UF_BASEURL')) {
  define( 'CIVICRM_UF_BASEURL'      , $civiDomain);
}
 ...
 */
 ...
 $civicrm_setting['Directory Preferences']['uploadDir'] = $tempDir;
 ...
 $civicrm_setting['Directory Preferences']['customTemplateDir'] = '[cms.root]/civi_tpl/custom_tpl_47';

 // Override the Custom php path directory.
 $civicrm_setting['Directory Preferences']['customPHPPathDir'] = '[cms.root]/civi_tpl/custom_php';

 // Override the extensions directory.
 $civicrm_setting['Directory Preferences']['extensionsDir'] = '[cms.root]/extensions';
 ...
 $civicrm_setting['URL Preferences']['extensionsURL'] = '[cms.root]/extensions';
 ...
 // Default repository URL; civicrm.org will make its best recommendation for stable/compatible extensions
 $civicrm_setting['Extension Preferences']['ext_repo_url'] = 'https://civicrm.org/extdir/ver={ver}&cms={uf}';
